...
- **Section Atmart Data** (7 pages + `assets/data.css`) : `donnees.html` (hub, positionnement « données reconstituées, prêtes à décider ») ; `donnees-backbone.html` (**la doctrine** : 4 référentiels maîtres, 5 niveaux de valeur, schéma maître, statuts de valeur, matrice juridique, 12 packs priorisés) ; `datasets.html` (catalogue, 3 étagères) ; `donnees-solutions.html` (B2B/B2G/ONG : packs décisionnels, 6 licences) ; `donnees-campus.html` (accès académique) ; `donnees-parrainage.html` (sponsoring de datasets) ; `donnees-confiance.html` (charte sourcing/qualité/licences) ; `donnees-pack-geo-haiti.html` (**gabarit de fiche produit** — à répliquer pour chaque nouveau jeu).
  ...
  **Le Backbone est la règle de construction.** Tout nouveau jeu de données sur Haïti doit se raccrocher au référentiel territoire (`atmart_geo_id`, p-code OCHA) et reprendre le schéma maître ; chaque valeur porte son `statut_valeur` (O/A/H/I/M/E/N) et chaque source sa classe juridique (A→E) dans `data/atmart_registre_sources.csv`. Les scripts de construction vivent hors du repo, dans `Atmart_premium_datasets\` (`build_pack_geo.py`, `build_backbone_territoire.py`) — ils ne sont jamais livrés aux acheteurs.
  **Couche ouverte :** `data/` contient désormais des produits gratuits à part entière (référentiel territorial de base 192 lignes, registre des sources) et non plus seulement des échantillons de 10 lignes. Ne pas les supprimer : c'est la couche d'adoption prévue par la stratégie.
